...
def make_history(start=100, days=240, breakout=True):
    ...
    close = np.linspace(start, start * 1.35, days)
    high = close * 1.01
    low = close * 0.99
...
def test_detect_bullish_market_regime():
    assert detect_market_regime(make_history()) == MarketRegime.BULLISH


def test_candidate_detects_breakout_and_volume():
    c = candidate_from_history(UniverseMember("TEST", "Test Ltd"), make_history(start=300), MarketRegime.BULLISH)
    assert c is not None
    assert c.current_price >= c.resistance
    assert c.breakout_volume > c.avg_volume_20d
    assert c.target1 > c.current_price > c.stop_loss


class FakeProvider:
    name = "fake"
    def history_many(self, symbols, period, batch_size=40):
        data = {s: make_history(start=300 if s == "AAA" else 2500) for s in symbols}
        data["^NSEI"] = make_history(start=20_000)
        return data
...
def test_scan_keeps_quality_ranking_and_live_paper_split():
    ...
    classes = {p.symbol: p.classification for p in result.proposals}
    assert classes["AAA"] in {PortfolioType.LIVE, PortfolioType.NO_TRADE}
    assert classes["BBB"] in {PortfolioType.PAPER, PortfolioType.NO_TRADE}
    assert result.proposals == sorted(result.proposals, key=lambda p: p.ai_score, reverse=True)
